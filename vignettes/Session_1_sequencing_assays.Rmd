...
if (requireNamespace("nnSVG", quietly = TRUE)) {
  ...
  message("nnSVG is not installed in this environment; using case-study genes only.")
  svg_results <- data.frame(
    gene_name = svg_case_study_genes,
    rank = seq_along(svg_case_study_genes),
    LR_stat = NA_real_,
    padj = NA_real_,
    prop_sv = NA_real_
  )
...
umap_svg_df <- data.frame(
  UMAP1 = rep(reducedDim(spatial_data_svg, "UMAP")[, 1], times = length(svg_case_study_genes)),
  UMAP2 = rep(reducedDim(spatial_data_svg, "UMAP")[, 2], times = length(svg_case_study_genes)),
  gene = rep(svg_case_study_genes, each = ncol(spatial_data_svg)),
  logcounts = as.numeric(t(assay(spatial_data_svg, "logcounts")[svg_case_study_ids, ]))
)
